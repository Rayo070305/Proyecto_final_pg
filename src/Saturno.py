...
def main():
    ...
    planet_texture_id = load_texture("opengl project/image/saturn.jpg")[0]
    rings_texture_id, _, _ = load_texture("opengl project/image/rings.jpeg")  # Cambiar por la textura de los anillos de Saturno
    info_texture_id, info_width, info_height = load_texture("opengl project/image/saturnoinfo.jpg")
    ...
    while True:
        ...
        # Dibujar Saturno y sus anillos
        glPushMatrix()
        glTranslatef(4, 0, 0)  # Mover Saturno más a la derecha
        glRotatef(angle, 0, 1, 0)
        
        # Dibujar la esfera de Saturno
        glBindTexture(GL_TEXTURE_2D, planet_texture_id)
        draw_sphere(3.0, 30, 30, planet_texture_id)

        # Dibujar los anillos de Saturno
        glPushMatrix()
        glRotatef(90, 1, 0, 0)  # Rotar los anillos para que estén horizontales
        glBindTexture(GL_TEXTURE_2D, rings_texture_id)
        gluDisk(gluNewQuadric(), 3.2, 4.2, 30, 1)  # Ajusta el radio interior y exterior según sea necesario
        glPopMatrix()
        ...
        info_x = 150
        info_y = (display[1] - info_height) / 2
